...
# Persisted generated secrets (gitignored). The Atlas DB password is generated once and
# stored here so every subsequent run — and any bare `terraform apply` that sources this —
# reuses the SAME password. Without this the password lived only in the run's environment,
# so a re-apply saw it as empty and tried to reset the DB user to a null password (which
# breaks the app's connection string). Sourced early; written when a password is generated.
...
preflight() {
  ...
  # to the gitignored secrets file, so re-runs and later `terraform apply`s reuse it instead
  # of nulling the DB user. Precedence: explicit TF_VAR / tfvars > persisted file > generate.
  if [[ "$create" == "true" && -z "${TF_VAR_atlas_db_password:-}" && -z "$(tfvar atlas_db_password)" ]]; then
    ...
    umask 077
    { echo "# Generated by deploy.sh — DO NOT COMMIT. Reused across runs so the DB user password is stable."
      echo "TF_VAR_atlas_db_password=$TF_VAR_atlas_db_password"
    } > "$SECRETS_FILE"
    ok "generated Atlas DB password (28 alnum chars) → persisted to deploy/.deploy-secrets.env"
  elif [[ "$create" == "true" && -n "${TF_VAR_atlas_db_password:-}" ]]; then
    ok "using existing Atlas DB password (from env/${SECRETS_FILE##*/})"
  fi
