...
# v2 provider attribute syntax (verified against a working mongodbatlas 2.12 deploy).
...
# Access list: the peered AWS VPC CIDR (private path) + the deploy machine /32 (so the
# laptop can seed the cluster over the public path). One resource per CIDR (provider
# requirement) via for_each. depends_on the peering route so the private entry only opens
# once the route exists (ordering hygiene; the public /32 entry is independent).
